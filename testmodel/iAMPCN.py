...
if __name__ == '__main__':
    ...
    for seq_record in SeqIO.parse(test_file, "fasta"):
        ...
        unusual_aas = set(temp_seq.upper()).difference(set('ACDEFGHIKLMNPQRSTVWY'))
        if unusual_aas:
            flag = 1
            print(f'输入错误: 序列 {temp_id} 包含非常规氨基酸: {", ".join(unusual_aas)}')
            break
    
    if flag == 0:
        print("开始预测...")
        pred_df = predict(test_file)
        pred_df.to_csv(output_file_name + '.csv', index=False)
        print(f"预测结果已保存到 {output_file_name}.csv")
    
    # 计算并打印运行时间
    end_time = time.time()
    elapsed = end_time - start_time
    print(f"\n总运行时间: {timedelta(seconds=elapsed)}")