...
for train_file in /tsAMP/data/tsAMP-CS/train/*.xlsx; do
            echo "使用训练数据: $train_file"
            train_filename=$(basename "$train_file")
            train_filename1=$(basename "$train_file" .xlsx)
            CUDA_VISIBLE_DEVICES=0 python train.py --train_dir "$train_file" --filename_dir "$train_filename1"
            
           
            model_path="/tsAMP/model/tsAMP-CS/strain/${train_filename1}.pt"
            output_path="model"
